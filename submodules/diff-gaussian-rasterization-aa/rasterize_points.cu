...
torch::Tensor RasterizeGaussianContributionScoresCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const bool debug)
{
  ...
  if (residual.ndimension() != 2) {
    AT_ERROR("residual must have dimensions (image_height, image_width)");
  }

  const int P = means3D.size(0);
  const int H = image_height;
  const int W = image_width;
  if (target_start < 0 || target_count < 0 || target_start + target_count > P) {
    AT_ERROR("Invalid contribution score target range");
  }
  if (residual.size(0) != H || residual.size(1) != W) {
    AT_ERROR("residual shape must match raster image size");
  }
  ...
  torch::Tensor scores = torch::full({target_count}, 0.0, float_opts);
  ...
  if(P != 0 && target_count != 0)
  {
	  ...
	  CudaRasterizer::Rasterizer::contributionScores(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		prefiltered,
		antialiasing,
		residual.contiguous().data<float>(),
		target_start,
		target_count,
		scores.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
